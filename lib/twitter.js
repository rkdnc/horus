...
${chalk.gray(`+${'-'.repeat(140)}+`)}`)
  ...
  directMessage: (username, message) => {
      // Take a username, and return the user's ID.
      T.post('direct_messages/events/new', {"event": {
        "type": "message_create",
         "message_create": {
           "target": {"recipient_id": getId(username)}, 
      ...
      // Send message with ID and message to user
  }
}
